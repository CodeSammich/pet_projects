...
public class GrandmaThread extends Thread {
	...
	// now on
	// when we create a thread in a class we use "new GrandmaThread(this);"
	// instead.

	// ==================END IVAN COMMENT=====================

	public GrandmaThread(Activity activity) {
		// super();
		...
		if (testActivity() == 0) { //If Main Screen
			while(running) {
				try{
					...
					Thread.sleep(1000);//(long)Grandma.cps);

					MainActivity.count += Grandma.cps * Grandma.numberOfGrandmas;
			...
			while(running) {
				try{
					...
					Upgrades.cookieCount += Grandma.cps * Grandma.numberOfGrandmas;
					//forRefresh.refreshCookies(v);
				} catch (InterruptedException e) {
					//Do Nothing
				}
